...
func ConectaComDB() {
	...
	conexao := "host=172.23.0.2 user=root password=root dbname=root port=5432 sslmode=disable" // host capturado com: docker-compose exec postgress sh(esse é o serviço que vc deseja caputrar a porta), depois: hostname -i para exibir o ip
